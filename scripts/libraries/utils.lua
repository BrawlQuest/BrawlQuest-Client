...
drawVar()
addVar()
]] function getPaddedSize(max, padding, partitions)
...
function deleteText(text, amount) -- text = deleteText(text, amount)
    ...
    if byteOffset and string.len(text) > 0 then
...
function distanceToPoint(x, y, x2, y2)
    return math.sqrt(math.abs(x - x2) * math.abs(x - x2) + math.abs(y - y2) *
                         math.abs(y - y2))
end

function difference(a, b) return math.abs(a - b) end
...
function math.dist(x1, y1, z1, x2, y2, z2)
