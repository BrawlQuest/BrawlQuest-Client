...
function initLogin()
    ...
    launch = {
        logo = love.graphics
            .newImage("assets/ui/login/new-login/Luven Logo.png"),
        font = love.graphics
            .newFont("assets/ui/fonts/AmaticSC-Regular.ttf", 400),
        inAmount = 0,
        inCERP = 0,
        outAmount = 0,
        outCERP = 0
    ...
    loginText = {
        ...
        "Luven Interactive LTD 2020 - 2021", "", "Created By",
    ...
    charactersEntryImage = love.graphics.newImage(
                               "assets/ui/login/characterbackground.png")
    charactersButtonImage = love.graphics.newImage(
                                "assets/ui/login/characterwidget.png")
    ...
    basePanelImage = love.graphics.newImage(
                         "assets/ui/login/character base panel.png")
    lightningSfx = love.audio.newSource("assets/sfx/weather/lightning_b.ogg",
                                        "static")
...
function drawLogin()
    if loginPhase == "prelaunch" then
        ...
        local iwidth, iheight = launch.logo:getWidth() * imageScale,
                                launch.logo:getHeight() * imageScale
        love.graphics.setColor(1, 1, 1, 1)
        drawLoginBackground()
        love.graphics.setColor(1, 1, 1, launch.outCERP)
        love.graphics.draw(bqLogo, (love.graphics.getWidth() / 2) -
                               ((bqLogo:getWidth() * logoScale) * 0.5),
                           (love.graphics.getHeight() / 2) -
                               ((bqLogo:getHeight() * logoScale) * 0.5) - 100,
                           0, logoScale)
        love.graphics.printf(loadingText .. loadingDots, chatFont, 0,
        ...
        love.graphics.setColor(1 - launch.inCERP, 1 - launch.inCERP,
                               1 - launch.inCERP, launch.inCERP - launch.outCERP)
        love.graphics.draw(launch.logo, width * 0.5 - iwidth * 0.5,
                           height * 0.5 - iheight * 0.5 - 200 * imageScale, 0,
                           imageScale)
        love.graphics.print("LUVEN INTERACTIVE", launch.font, width * 0.5 -
                                (launch.font:getWidth("LUVEN INTERACTIVE") *
                                    imageScale) * 0.5,
                            height * 0.5 + 300 * imageScale, 0, imageScale)
    elseif loginPhase == "dev" then
        ...
        love.graphics.printf(
            "Welcome to Dev Mode!\nPress LEFT or \"A\" to Login with Steam ID\nPress RIGHT or \"D\" to Login with anything else\nOtherwise you can quick login like before\n" ..
                loadingDots, chatFont, 0, love.graphics.getHeight() * 0.5 + 70,
            love.graphics.getWidth() / 1, "center", 0, 1)
    else
        drawLoginBackground()
...
function drawLoadingText(text)
    ...
    love.graphics.draw(bqLogo, (love.graphics.getWidth() / 2) -
                           ((bqLogo:getWidth() * logoScale) * 0.5),
                       (love.graphics.getHeight() / 2) -
                           ((bqLogo:getHeight() * logoScale) * 0.5) - 100, 0,
                       logoScale)
    love.graphics.printf(text .. loadingDots, chatFont, 0,
                         love.graphics.getHeight() * 0.5 + 70,
                         love.graphics.getWidth() / 1, "center", 0, 1)
end
