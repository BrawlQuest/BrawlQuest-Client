...
function drawNPCChatBackground(x, y)
    ...
    if not worldImg[npcChat.img] then
        if love.filesystem.getInfo(npcChat.img) then
            worldImg[npcChat.img] = love.graphics.newImage(npcChat.img)
        else
            worldImg[npcChat.img] = love.graphics.newImage("assets/error.png")
        end
    end
    love.graphics.draw(worldImg[npcChat.img], x + 128, y + 126, 0, 4, 4)
...
function checkNPCChatMousePressed()
    ...
    for i, v in pairs(npcChat.conversation[currentConversationStage].options) do
        ...
        if isMouseOver(tx, ty, 133, rHeight + 5) then
            ...
            if currentConversationStage == 1 then
                showNPCChatBackground = false
