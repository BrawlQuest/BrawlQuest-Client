...
function initWorldTable(world)
    ...
    for i, tile in ipairs(world) do
        ...
        local x, y = math.floor((tile.X) / chunkSize),
                     math.floor((tile.Y) / chunkSize)
        -- add tile to chunk, works perfectly
        if not worldChunks[x .. "," .. y] then
            worldChunks[x .. "," .. y] = {}
        end
        if player.world == 0 then
            worldChunks[x .. "," .. y][#worldChunks[x .. "," .. y] + 1] = copy(
                                                                              tile)
        end
...
function tickWorld()
    ...
    player.wx, player.wy = math.floor((player.x + halfChunk) / chunkSize),
                           math.floor((player.y + halfChunk) / chunkSize)
...
function createWorld()
    ...
    if not love.filesystem.getInfo("img") then
        ...
        if not loadedChunks[chunk.key] then
            if worldChunks[chunk.key] then
                for i, v in ipairs(worldChunks[chunk.key]) do
                    ...
                    if v.GroundTile and v.GroundTile ~= "" then
                        serverTiles[v.X .. "," .. v.Y] = true
                    end
                end
            end
...
function addLeavesAndLights(v)
    ...
    if lightGivers[v.ForegroundTile] then -- and not lightSource[v.X .. "," .. v.Y] then
        ...
        Luven.addNormalLight(16 + (v.X * 32), 16 + (v.Y * 32),
                             lightGivers[v.ForegroundTile].color,
                             lightGivers[v.ForegroundTile].brightness)
    end
end
...
function drawChunks(cx, cy)
    ...
    if info then
        ...
        if chunkTiles then
            for i, v in ipairs(chunkTiles) do drawTile(v, cx, cy) end
        end
...
function drawTile(v, cx, cy)
    ...
    -- draw noise
    if v.Color then
        love.graphics.setColor(v.Color, v.Color, v.Color)
    else
    ...
    if worldLookup[v.X .. "," .. v.Y - 1] and
        (isTileWall(worldLookup[v.X .. "," .. v.Y - 1].ForegroundTile) or
            isTileWall(worldLookup[v.X .. "," .. v.Y - 1].GroundTile)) and
        not isTileWall(v.ForegroundTile) then
        love.graphics.rectangle("fill", x * 32, y * 32, 32, 16)
    elseif (isTileWall(v.GroundTile) or isTileWall(v.ForegroundTile)) and
        not worldLookup[v.X .. "," .. v.Y + 1] then -- no tile below us but we still need to cast a shadow
        love.graphics.rectangle("fill", x * 32, y * 32, 32, 16)
    end
    ...
    -- draw foreground
    if foregroundAsset ~= backgroundAsset and worldImg[foregroundAsset] then
        love.graphics.draw(worldImg[foregroundAsset], (x) * 32, (y) * 32)
    end
end
...
--         v = explode(key, ",")
--         love.graphics
--             .draw(img, v[1] * (32 * chunkSize), v[2] * (32 * chunkSize))
--     end
-- end
...
function drawSimplexNoise(x, y)
    ...
    local noises = {
        ...
        love.math.noise((x * 0.07) - player.world, (y * 0.07) + player.world) *
            0.1,
        love.math.noise((x * 0.06) - player.world, (y * 0.06) + player.world),
        love.math.noise((x * 0.5) - player.world, (y * 0.5) + player.world) *
            0.2,
        love.math.noise((x * 0.06) - player.world, (y * 0.06) + player.world),
        love.math.noise((x * 0.5) - player.world, (y * 0.5) + player.world) *
            0.2
    }
    islandNoise = math.clamp(0.8, noises[1] - noises[2], 1) +
                      ((noises[3] - noises[4]) * 2 - 1) * 0.1
    if islandNoise > 0.85 then
        noise = islandNoise
    else
        noise = 1 - ((noises[5] - noises[6]) * noiseFactor)
    end
    noise = islandNoise
